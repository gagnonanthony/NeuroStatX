...
def plot_parallel_plot(
    X, labels, output, mean_values=False, title="Parallel Coordinates plot."
):
    """
    ...
    Args:
        ...
        title (str, optional): _description_.
                                Defaults to 'Parallel Coordinates plot.'.
...
def plot_grouped_barplot(X, labels, output, title="Barplot", annotation=None):
    ...
    with plt.rc_context(
        ...
        annotator.configure(
            test="t-test_ind", text_format="star", show_test_name=False,
            verbose=0
        )
