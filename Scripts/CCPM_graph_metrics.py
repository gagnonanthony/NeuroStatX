...
from CCPM.network.metrics import (get_metrics_ops,
                                  get_metrics_docs)
...
@app.command()
def main(
        operation: Annotated[List[str], typer.Argument(help=__doc__,
                                                       show_choices=True,
                                                       show_default=False,
                                                       rich_help_panel='Operations Options')],
        out_file: Annotated[str, typer.Option(help='Path and name of the file containing the metrics for each nodes. '
    ...
    if len(operation) < 3:
        sys.exit('Incorrect number of input provided. Please see {}'.format(__doc__))
