...
def main(
        ...
        disable_plotting: Annotated[bool, typer.Option('--disable_plotting',
                                                       help='If used, will disable all plotting and simply output '
                                                            'tables.',
                                                       rich_help_panel='Essential Files Options',
                                                       show_default=True)] = False,
        context: Annotated[ContextChoices, typer.Option(help='Context to use as plot style.',
                                                        rich_help_panel='Matplotlib Options',
                                                        case_sensitive=False)] = ContextChoices.poster,
        font_scale: Annotated[int, typer.Option(help='Font size to use in plots.',
                                                rich_help_panel='Matplotlib Options')] = 1,
        cmap: Annotated[str, typer.Option(help='Cmap to be used in the heatmap plot.',
                                          rich_help_panel='Matplotlib Options')] = 'mako',
        annotate: Annotated[bool, typer.Option('--annotate',
                                               help='If true, heatmap will show pearson correlation coefficient '
                                                    'in the center of each square.',
                                               rich_help_panel='Matplotlib Options')] = False,
    ...
    description_df.to_excel(f'{out_folder}/descriptive_statistics.xlsx', header=True, index=True)
    ...
    if disable_plotting:
        ...
        plot_distributions(variable_to_plot, path_plot, context=context, font_scale=font_scale)
    ...
    corr_mat.to_excel(f'{out_folder}/correlation_coefficient.xlsx', header=True, index=True)

    # Generating basic recommendations. TODO: Set up a easy to interpret report with useful distribution infos.
#    if report:
