...
def test_compute_graph_network():
    ...
    in_mat = os.path.join(get_home(), 'data/membership.npy')
    out_folder = os.path.join(get_home(), 'data/compare_graph/')

    ret = runner.invoke(app, ['--in-graph1', in_graph, '--in-matrix', in_mat, '--percentile', 80, 
                              '--in-graph2', in_graph, '--out-folder', out_folder, '-f'])
